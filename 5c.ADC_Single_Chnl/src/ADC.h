...
{
	ADC1->CFGR |= ADC_CFGR_CONT; 			// ADC ContinuousConvMode Enable
	ADC1->IER |= ADC_IER_EOC;				//Fire interrupt, sw to ADC1_2_IRQHandler()
	NVIC_EnableIRQ(ADC1_IRQn);				//NVIC is interrupt handler
	NVIC_SetPriority(ADC1_IRQn, 0);			//set priority
}
